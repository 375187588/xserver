...
local function get_access_token(code)
    skynet.send("xlog","lua","get_access_token code="..code)
    if not host_ip then
    ...
    skynet.send("xlog", "lua", "status="..status.." body="..body)
    return cjson.decode(body)
end

local function refresh_access_token(refresh_token)
    skynet.send("xlog", "lua", "refresh_access_token token="..refresh_token)
    if not host_ip then
        host_ip = dns.resolve(host)
    end
