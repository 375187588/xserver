...
function M:onGameScene(userid, _)
    ...
    data.nCardCount = {define.game_cardcount, define.game_cardcount, define.game_cardcount}
...
function M:GAME_OutCardReq(userid, msg)
    ...
    if first_out == 1 and self.nPlayerCount == 3 and self.nGameCount == 1 then
        if #self.tTurnCardData == 0 and #self.tCardData[nOutCardSeat] == define.game_cardcount then
            local bFind = false
            -- 判断出的牌是否含有黑桃3
	...
	if self.nCurrentSeat == define.invalid_seat then
		-- 有炸弹
		if self.nBombSeat <= define.game_player then
...
function M:gameBegin()
    ...
    for nIndex, nValue in ipairs(tAllCard) do
        local nPlayerIndex = math.modf((nIndex - 1) / define.game_cardcount) + 1
		...
		for i = 1, self.nPlayerCount do
			...
			if i ~= nSeat then
			    ...
			    if nCardCount == 1 then
				    tGameEnd.nGameScore[i] = 0
                -- 全关双倍
			    elseif nCardCount == define.game_cardcount then
