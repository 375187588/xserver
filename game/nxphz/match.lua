...
function M:init()
    ...
    self.nGameCount = 0
    -- 游戏总局数
    self.nTotalGameCount = room.get("room_card")
    -- 玩家人数
...
function M:GAME_OutCardReq(userid, msg)
    ...
    -- 无效牌
    if not logic:isValidCard(nCardData) then
        return
    end
    ...
    if not logic:checkRemoveCard(self.tCardData[nOutCardSeat], tCardData) then
        return
    end

    -- 移除牌
    logic:removeCard(self.tCardData[nOutCardSeat], tCardData)
...
function M:GAME_OperateCardReq(userid, msg)
	...
	if self.tResponse[nOperateCardSeat] or
        ...
        skynet.call("xlog", "lua", "log", string.format(
            "nRoomID=%d,tResponse[%d]=%d",
            self.nRoomID,
            nOperateCardSeat,
            self.tResponse[nOperateCardSeat]))
		return
	end
	...
	while bCheck do
        ...
        -- 吃了还有剩余
        if not tChiCardData[nCardData] and self.tCardData[nOperateCardSeat][nCardData] > tChiCardData[nCardData] then
            skynet.call("xlog", "lua", "log", string.format(
    ...
    for i = nFirstCurrentSeat, nFirstCurrentSeat + (self.nPlayerCount - 1) do
        local nIndex = i % self.nPlayerCount
        local nSeat = nIndex ~= 0 and nIndex or self.nPlayerCount
	...
	if nMaxTargetAction ~= define.ACK_NULL and
        ...
        skynet.call("xlog", "lua", "log", string.format(
            "nRoomID=%d,nOperateCardSeat=%d,nTargetSeat=%d,nMaxTargetSeat=%d,nMaxTargetAction=%d,tResponse[%d]=%d",
            self.nRoomID,
            nOperateCardSeat,
            nTargetSeat,
            nMaxTargetSeat,
		...
		if self.bWaitSendUserHu then
            ...
            for i = 1, self.nPlayerCount do
				...
				if not (i == self.nOutCardSeat or self.bSendNotify) then
                    ...
                    if logic:getHuCardInfo(tHuCardInfo) then
                        -- 设置玩家可胡
            ...
            skynet.call("xlog", "lua", "log", string.format(
                "nRoomID=%d,bWaitSendUserHu=%s,nOutCardSeat=%d,nOutCardData=%d,bFindHu=%d,%s",
                self.nRoomID,
			...
			if bFindHu then
                ...
                skynet.call("xlog", "lua", "log", string.format(
                    "NULL nRoomID=%d,nOutCardSeat=%d,nOutCardData=%d,nResumeSeat=%d,tPaoPai[%d]=%d,bTiCard=%s",
                    self.nRoomID,
                    self.nOutCardSeat,
                    self.nOutCardData,
                    self.nResumeSeat,
                    nCurrentHuSeat,
                    self.tPaoPai[nCurrentHuSeat],
	...
	if (nTargetAction & define.ACK_CHI) ~= 0 then
		...
		for i = 1, nResultCount do
            ...
            table.insert(self.tWeaveItemArray, tWeave)
    ...
    skynet.call("xlog", "lua", "log", "ACK_CHIHU nRoomID="..self.nRoomID..",bUserAction="..utils.table_2_str(bUserAction))
	-- 派发扑克
	if not bUserAction then
		-- 杀掉发牌定时器
        if self.nDispatchCardTimerID then
...
function M:gameBegin()
    ...
    -- 打乱
    logic:shuffle(self.tRepertoryCard)

    self.nLeftCardCount = #self.tRepertoryCard
    for i = 1, self.nPlayerCount do
        self.nLeftCardCount = self.nLeftCardCount - (define.MAX_COUNT - 1)
        for j = 1, define.MAX_COUNT-1 do
            local nCardData = self.tRepertoryCard[self.nLeftCardCount + j]
            self.tCardData[i][nCardData] = self.tCardData[i][nCardData] + 1
        end
    end

    local nBankCard = self.tRepertoryCard[self.nLeftCardCount]
	...
	self.nOutCardData = define.INVALID_CARD
...
function M:onContinueCard(nCurrentSeat)
	...
	-- 还原用户
	self.nResumeSeat = self:getNextDeskStation(nCurrentSeat)
    -- 杀掉发牌定时器
    if self.nDispatchCardTimerID then
        self.timer_mgr:remove(self.nDispatchCardTimerID)
        self.nDispatchCardTimerID = nil
...
function M:dispatchCard()
    ...
    -- 有出牌玩家,记录玩家出牌
	if self.nOutCardSeat ~= define.INVALID_SEAT and logic:isValidCard(self.nOutCardData) then
		-- 自己臭牌记录
    ...
    -- 发送扑克
    local nSendCardData = self.tRepertoryCard[self.nLeftCardCount]
    self.nLeftCardCount = self.nLeftCardCount - 1
    -- 发牌数量增加
	...
	-- 提牌判断
	if logic:isTiPaoCard(self.tCardData[nCurrentSeat], nSendCardData) then
		bPerformAction = true
	end
	-- 偎牌变提
	...
	-- 偎牌判断
	if not bPerformAction and logic:isWeiPengCard(self.nCardData[nCurrentSeat], nSendCardData) then
		bPerformAction = true
	end
...
function M:sendOutCardNotify(nCurrentSeat, bOutCard)
    ...
    player_mgr:broadcast("nxphz.GAME_OutCardAck", tData)
    -- 回放数据
    table.insert(self.record.game.data, {time = os.time(), name = "nxphz.GAME_OutCardAck", msg = tData})
...
function M:estimateUserRespond(nCenterSeat, nCenterCard, bDispatch)
	...
	local bPerformAction = false
	-- 提牌判断
	if not self.bEstimate and bDispatch and logic:isTiPaoCard(self.tCardData[nCenterSeat], nCenterCard) then
		-- 设置组合
        local tWeave =
        {
	...
	if not bPerformAction and not self.bEstimate and bDispatch and
        logic:isWeiPengCard(self.tCardData[nCenterSeat], nCenterCard) then
		-- 设置扑克
		self.tCardData[nCenterSeat][nCenterCard] = 0
	...
	for i = 1, self.nPlayerCount do
		...
		if i ~= nCenterSeat and not self.tPaoPai[i] then
            -- 跑牌判断
            if logic:isTiPaoCard(self.tCardData[i], nCenterCard) then
                -- 变量定义
                local nActionSeat = i
	...
	for i = 1, self.nPlayerCount do
		...
		if not self.tPaoPai[i] then
            for _, tWeaveItem in pairs(self.tWeaveItemArray[nCenterSeat]) do
                -- 转换判断
                local bChangeWeave = false
                if nCenterCard == tWeaveItem.tCardData[1] then
                ...
                if bChangeWeave then
                    ...
                    local tWeave =
                    {
                        nWeaveKind = nWeaveKind,
                        nCenterCard = nCenterCard,
                        tCardData = {nCenterCard, nCenterCard, nCenterCard, nCenterCard},
                    }
                    table.insert(self.tWeaveItemArray, tWeave)

                    if tWeave.nWeaveKind == define.ACK_PAO then
                        -- 设置跑牌
                        self.tPaoPai[nActionSeat] = true
                    end
                    ...
                    -- 发送数据
                    if tWeave.nWeaveKind == define.ACK_TI then
                        -- 发送数据
                        local tData =
                        {
                            nCurrentSeat = nActionSeat,
                        ...
                        local tData =
                        {
                            ...
                            nCardData = nCenterCard,
                            nRemoveCount = 0,
                            bWeiToPao = tWeave.nWeaveKind == define.ACK_WEI,
                        }
                        player_mgr:broadcast("nxphz.GAME_PaoCardAck", tData)
                        -- 回放数据
                        table.insert(self.record.game.data, {
                    ...
                    self.nOutCardSeat = define.INVALID_SEAT
                    -- 出牌提示
                    self:sendOutCardNotify(define.INVALID_SEAT, self:isAllowOutCard(define.INVALID_SEAT))
			...
			if not (not bDispatch and i == nCenterSeat) then
                ...
                if nTotalCardCount ~= 2 then
                    ...
                    if not self.tAbandonCard[i][nCenterCard] then
                        -- 碰牌判断
                        if logic:isWeiPengCard(self.tCardData[i], nCenterCard) then
                        ...
                        -- 吃牌需要数量
                        local bEat, tEatCardData = logic:isChiCard(self.tCardData[i], nCenterCard)
                        -- 吃牌需要的牌要比手牌少
...
function M:updateHuXi(nCurrentSeat)
	...
	local nHuXi = 0
	for _, tWeave in pairs(self.tWeaveItemArray[nCurrentSeat]) do
        nHuXi = nHuXi + logic:getWeaveHuXi(tWeave)
    end
	-- 设置胡息
