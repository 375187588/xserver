...
function CMD.join(info)
    -- 检测加入玩家
    if not player_mgr:check_add_info() then
        return
    end
    player_mgr:add_info(info.player)
end
...
local function enter(msg)
    ...
    if room.get("master_id") ~= msg.userid then
        skynet.send("roommgr", "lua", "join_room_result", {roomid=msg.roomid,userid=msg.userid})
    end
